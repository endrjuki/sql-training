...
const insertMovieKeywords = (
  ...
  return (`
    ...
    keywords.map(keywordName =>          
      `(${movieId}, ${keywordRows.filter(row => row.keyword == keywordName)[0].id})`
    ).join(", ")    
  );
};
...
const insertMovieProductionCompanies = (
  ...
  return (`
    ...
    productionCompanies.map(companyName =>        
      `(${movieId}, ${productionCompanyRows.filter(row => row.company_name == companyName)[0].id})`
    ).join(", ")    
  );
};
  ...
  it(
    ...
    async done => {
      ...
      const productionCompanyRows = (await db.selectMultipleRows(
        `select * from production_companies`)) as ProductionCompanyRow[];
      const moviesByImdbId = _.groupBy(await CsvLoader.movies(), "imdbId");
